...
export default {
  ...
  watch: {
    ...
    internalValue() {
      let rawData = null;
      if (this.multiple) {
        rawData = this.internalValue.map(id =>{
          return this.getNode(id).raw;
        });
      } else {
        rawData = this.internalValue.map(id => this.getNode(this.getValue()).raw)[0];
      }
      this.$emit('input', this.getValue(), this.id, rawData)
    },
  ...
  methods: {
    ...
    createFallbackNode(id) {
      ...
      const label = this.normalizer(raw).label || `${id}`
      const fallbackNode = {
        id,
        label,
        ancestors: [],

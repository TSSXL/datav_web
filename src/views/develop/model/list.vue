  ...
  <div class="app-container list">
    ...
    <el-table :data="list" v-loading.body="listLoading" element-loading-text="拼命加载中"
      ...
      <el-table-column label="操作" width="318">
        <template slot-scope="scope">
          <el-button type="text" icon="fa fa-navicon" @click="handleList(scope.row)" size="mini"> 列表</el-button>
          <el-button type="text" icon="fa fa-edit" @click="handleUpdate(scope.row)" size="mini"> 修改</el-button>
          <el-button type="text" icon="fa fa-list-ol" @click="handleDataField(scope.row)" size="mini"> 数据字段</el-button>
          <el-button type="text" icon="fa fa-file-text-o" @click="handlePage(scope.row)" size="mini"> 页面</el-button>
    ...
    <div v-show="!listLoading" class="pagination-container">
      <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange"
                     ...
                     :page-size="paging.pageSize" layout="total, sizes, prev, pager, next" :total="total">
      </el-pagination>
    </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      handleList(row) {
        this.$router.push({ path: '/develop/collection/list/' + row._id })
      },
      handleDataField(row) {
        this.$router.push({ path: '/develop/dataField/index/' + row._id })
      },
      handlePage(row) {
        this.$router.push({ path: '/develop/page/list/' + row._id })
      }
    },
    beforeCreate() {
    }

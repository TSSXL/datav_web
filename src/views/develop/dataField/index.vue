<template>
  <div class="app-container list">
    <el-row :gutter="20">
      ...
      <el-col :span="18">
        <el-button type="text" @click="initSysFields" size="mini"> 初始化系统字段 </el-button>
    ...
    <el-row :gutter="30">
      ...
      <span class="custom-tree-node" slot-scope="{ node, data }">
        ...
        <span>
          ...
          <el-button
            v-if="(data.sysField && data.name ==='properties') || (!data.sysField && data.type ==='Mixed')"
            type="text"
            size="mini"
...
<script>
  ...
  import { addItem } from '@/views/base/server/api'
  import { sysFields } from '@/views/develop/page/data/sysFields'
  ...
  export default {
    ...
    methods: {
      initSysFields() {
        for (var field of sysFields) {
          field['parentId'] = 'root'
          field['group'] = this.group

          addItem(this.MODEL_NAME, field).then(res => {
            message(res)
            if (!res.success) {
              return
            }

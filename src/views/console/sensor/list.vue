...
<template>
  <div class="app-container list">
    ...
    <el-table :data="list" v-loading.body="listLoading" element-loading-text="拼命加载中"
      ...
      <el-table-column key='field' 
        show-overflow-tooltip
        label="消息字段" 
        width="220" align="center" prop="field">
        <template slot-scope="scope">  
      ...
      <el-table-column key='unit' 
        show-overflow-tooltip
        label="单位" 
        width="120" align="center" prop="unit">
        <template slot-scope="scope">  
...
export default {
  ...
  methods: {
    ...
    handleCreate() {
      this.$router.push({ path: "/console/sensor/add" });
    },
    handleUpdate(row) {
      this.$router.push({ path: "/console/sensor/edit/" + row.id });
    },
    handleDelete(row) {

...
<template>
  <div class="app-container list">
        ...
        <div class="filter-container">
            ...
            <span>
                <el-input @keyup.enter.native="handleFilter" style="width: 200px;" 
                  class="filter-item" placeholder="系统名称" v-model="objQuery[0].name.$regex" size="mini">
                </el-input>
            </span>
    ...
    <el-table :data="list" v-loading.body="listLoading" element-loading-text="拼命加载中"
      ...
      <el-table-column key='adress' 
        
        label="访问地址" 
        width="220" align="center" prop="adress">
        <template slot-scope="scope">  
      ...
      <el-table-column key='tag' 
        show-overflow-tooltip
        label="标签" 
        width="180" align="center" prop="tag">
        <template slot-scope="scope">  
...
export default {
  ...
  methods: {
    ...
    handleCreate() {
      this.$router.push({ path: "/console/app/add" });
    },
    handleUpdate(row) {
      this.$router.push({ path: "/console/app/edit/" + row.id });
    },
    handleDelete(row) {

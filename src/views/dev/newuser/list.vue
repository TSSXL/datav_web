  ...
  <div class="app-container list">
    ...
    <el-table :data="list" v-loading.body="listLoading" element-loading-text="拼命加载中"
      ...
      <el-table-column key='desc' 
        show-overflow-tooltip
        label="描述" sortable="custom"
        width="120" align="center" prop="desc">
        <template slot-scope="scope">  
      ...
      <el-table-column key='age' 
        show-overflow-tooltip
        label="年龄" 
        width="120" align="center" prop="age">
        <template slot-scope="scope">  
...
export default {
  ...
  methods: {
    ...
    handleCreate() {
      this.$router.push({ path: "/dev/newuser/add" });
    },
    handleUpdate(row) {
      this.$router.push({ path: "/dev/newuser/edit/" + row.id });
    },
    handleDelete(row) {

  ...
  <div class="app-container list">
        ...
        <div class="filter-container">
            <span>
                <el-input @keyup.enter.native="handleFilter" style="width: 200px;"
                  class="filter-item" placeholder="姓名" v-model="objQuery[0].name.$regex" size="mini">
                </el-input>
            </span>
    ...
    <el-table :data="list" v-loading.body="listLoading" element-loading-text="拼命加载中"
      ...
      <el-table-column key='category'
        ...
        label="目录"
        width="240" align="center" prop="category">
        <template slot-scope="scope">
           <el-tag v-for="tag in scope.row.category" :key="tag" size="small">{{tag}}</el-tag>
      ...
      <el-table-column  label="操作" width="172" align="center" >
        <template slot-scope="scope">
          <el-button type="text" icon="fa fa-send" @click="handleCommit(scope.row)" size="mini"> 提交</el-button>
...
export default {
  ...
  methods: {
    handleCommit() {
      const processName = 'serviceApprove'
    ...
    handleCreate() {
      this.$router.push({ path: "/dev/example/add" });
    },
    handleUpdate(row) {
      this.$router.push({ path: "/dev/example/edit/" + row.id });
    },
    handleDelete(row) {

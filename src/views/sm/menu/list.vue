  ...
  <div class="app-container list">
    ...
    <el-table :data="list" v-loading.body="listLoading" element-loading-text="拼命加载中"
      ...
      <el-table-column label="操作" width="256" align="center">
        <template slot-scope="scope">
          <el-button type="text" icon="fa fa-edit" @click="handleUpdate(scope.row)" size="mini"> 修改组</el-button>
          <el-button type="text" icon="fa fa-cog" @click="handleSetting(scope.row)" size="mini"> 设置</el-button>
          <el-button type="text" icon="fa fa-cog" @click="handlePermisson(scope.row)" size="mini"> 权限</el-button>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      handlePermisson(row) {
        this.$router.push({ path: '../permission/menu/' + row.value })
      },
      handleCopy(row) {
        this.$router.push({ path: 'copy/' + row.value })
      },
      handleUpdate(row) {

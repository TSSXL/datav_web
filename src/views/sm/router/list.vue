  ...
  export default {
    ...
    methods: {
      ...
      handleCreate() {
        ...
        //this.$router.push({ name: 'sm_dataGroup_add', params: { module: this.module }})
      },
      handleSetting(row) {
        this.$router.push({ path: 'index/' + row.value })
        //this.$router.push({ name: 'sm_router_index', params: { group: row.value } })
      },
      handleCopy(row) {
        this.$router.push({ path: 'copy/'+ row.value })
       // this.$router.push({ name: 'sm_router_copy', params: { group: row.value } })
      },
      handleUpdate(row) {
      ...
      delete(row) {
        removeGroup(row._id, row.value).then(res => {
          this.$message({
            type: 'success',
            message: '删除成功!'
          })
